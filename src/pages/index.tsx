...
import { formatNumberDownRound, telegramInitDataRawToObject } from '@/utils'
...
export const IndexPage: FC = () => {
  ...
  const [userInfo, setUserInfo] = useState<UserInfo | undefined | any>(
    undefined
  )
  ...
  useEffect(() => {
    if (userInfo) {
      setTotalBalance(get(userInfo, 'balance', 0))
      setEnergy(get(userInfo, 'incubationCanSpent', 0))
        ...
        fetch(API_GET_INFO, {
          ...
          .then(async (res) => {
            ...
            if (res.status === 200) {
              ...
              console.log('asahsas', userData.data)
      ...
      <div className='top'>
        <div className='inner'>
          <div className='point'>
            ...
            <span>{formatNumberDownRound(totalBalance)}</span>
          </div>
          ...
          <div className='info'>
            <div className='level'>
              ...
              <span>{get(userInfo, 'level', 1)}</span>
            </div>
            <div className='tap'>
              <img src='/assets/images/tap.png' alt='' />
              <span>Tap: {formatNumberDownRound(totalBalance)}</span>
            </div>
          </div>
        </div>
      </div>
